/**
 * Helper function to check for missed reminders for a specific user
 * Can be called from various places (reminder send, user login, etc.)
 * to catch missed reminders without needing frequent cron jobs
 */
...
export async function checkMissedRemindersForUser(
  userId: string
): Promise<{ processed: number; errors: number }> {
  const supabase = createServiceClient();
  const now = new Date();

  // Find reminders that are overdue (remind_at < now) and still pending for this user
  ...
  if (error) {
    console.error(`Failed to fetch missed reminders for user ${userId}:`, error);
