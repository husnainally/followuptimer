...
export default function RemindersPage() {
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      {loading ? (
        <Card className="min-h-64 border border-border/60 shadow-sm">
          <CardContent className="flex h-64 items-center justify-center">
            <p className="text-muted-foreground">Loading reminders...</p>
          </CardContent>
        </Card>
      ) : reminders.length === 0 ? (
        <Card className="bg-[#FAFAFA] border-border min-h-96">
        ...
        <Card className="min-h-96 border border-border/60 shadow-sm">
          <CardHeader className="space-y-6">
            ...
            <div className="flex flex-wrap items-end gap-3">
              <div className="relative flex-1 min-w-[220px]">
                ...
                <Input
                  ...
                  className="h-11 rounded-xl border-border/60 bg-white pl-9 pr-4 focus-visible:ring-2 focus-visible:ring-primary/20"
                />
              </div>
              <FilterChip
                label="Tone"
            ...
            <div className="flex flex-wrap gap-2 rounded-full w-fit border border-border/70 bg-[#F3F4F6]  p-1">
              {statusTabs.map((tab) => {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className="flex h-11 items-center justify-between rounded-xl border-border/60 bg-white px-4 text-sm"
        >
          <div className="flex items-center gap-2 text-muted-foreground">
            <Filter className="h-4 w-4" />
            <span className="font-medium">{label}</span>
          </div>
          <div className="flex items-center gap-1 text-sm text-foreground capitalize">
            {currentLabel}
        ...
        <div className="flex flex-col gap-1">
          {options.map((option) => (
            <button
              ...
              className={cn(
                "flex w-full items-center rounded-md px-3 py-2 text-sm capitalize",
                option === value ? "bg-muted/80 text-foreground" : "text-muted-foreground hover:bg-muted/50"
              )}
            >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "bg-foreground text-background shadow-sm" : "bg-white text-muted-foreground border-border",
